<x-app-layout>
    <div class="py-10 bg-gray-100 dark:bg-slate-900 min-h-screen" x-data="{ showDeleteModal: false, deleteForm: null }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">

            @if (session('success'))
                <div class="p-4 bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-600 text-green-700 dark:text-green-300 rounded-lg">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200/60 dark:border-slate-700 overflow-hidden">
                <div class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100 dark:divide-slate-700 text-center">
                    <div class="p-6 hover:bg-gray-50/50 dark:hover:bg-slate-700/50 transition">
                        ...
                        <span class="block text-3xl font-extrabold text-green-600 dark:text-green-400">Rp {{ number_format($totalIncome, 0, ',', '.') }}</span>
                    </div>
                    <div class="p-6 hover:bg-gray-50/50 dark:hover:bg-slate-700/50 transition">
                        ...
                        <span class="block text-3xl font-extrabold text-red-600 dark:text-red-400">Rp {{ number_format($totalExpense, 0, ',', '.') }}</span>
                    </div>
                    <div class="p-6 bg-indigo-50/30 dark:bg-indigo-900/20">
                        ...
                        <span class="block text-3xl font-extrabold text-indigo-600 dark:text-indigo-400">Rp {{ number_format($activeWalletBalance, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-200/60 dark:border-slate-700 space-y-4">
                    <div class="border-b border-gray-100 dark:border-slate-700 pb-3">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">Catat Transaksi Baru</h3>
                        <p class="text-xs text-gray-400 dark:text-slate-400 mt-0.5">Input data pemasukan atau pengeluaran secara manual.</p>
                    </div>

                    @if($wallets->isEmpty())
                        <div class="text-center py-6">
                            <p class="text-sm text-gray-500 dark:text-slate-400 mb-4">Anda belum memiliki dompet. Buat dompet terlebih dahulu.</p>
                            <a href="{{ route('wallets.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 dark:bg-indigo-600 text-white text-sm font-semibold rounded-md hover:bg-indigo-700 dark:hover:bg-indigo-700">
                                + Buat Dompet
                            </a>
                        </div>
                    @else
                        <form method="POST" action="{{ route('transactions.store') }}" class="space-y-4">
                            ...
                            <div class="flex flex-col justify-between gap-1">
                                ...
                                <x-select id="form_wallet" name="wallet_id" :options="$wallets->map(fn($w) => ['value' => $w->id, 'label' => $w->name])" value="{{ $activeWallet?->id }}" class="mt-1" placeholder="Pilih Dompet" />
                            </div>

                            <div class="grid grid-cols-2 gap-3">
                                <div class="flex flex-col justify-between gap-1">
                                    <x-input-label for="form_type" :value="__('Tipe')" />
                                    <x-select id="form_type" name="type" :options="[['value' => 'expense', 'label' => 'Pengeluaran'], ['value' => 'income', 'label' => 'Pemasukan']]" class="mt-1 block w-full text-sm" placeholder="Tipe" />
                                </div>
                                <div class="flex flex-col justify-between gap-1">
                                    <x-input-label for="amount" :value="__('Nominal (Rp)')" />
                                    <x-text-input id="amount" name="amount" type="text" inputmode="numeric" class="block w-full text-sm idr-input" required placeholder="Rp 0" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="flex flex-col justify-between gap-1">
                                    ...
                                    <x-select id="category" name="category_id" :options="($categories ?? collect())->map(fn($c) => ['value' => $c->id, 'label' => $c->name])" class="mt-1 block w-full text-sm" placeholder="Pilih Kategori" />
                                </div>
                                <div class="flex flex-col justify-between gap-1">
                                    <x-input-label for="transaction_date" :value="__('Tanggal')" />
                                    <x-text-input id="transaction_date" name="transaction_date" type="date" class="block w-full text-sm" required :value="now()->format('Y-m-d')" />
                                </div>
                            </div>

                            <div>
                                <x-input-label for="description" :value="__('Keterangan / Catatan')" />
                                <textarea id="description" name="description" rows="2" class="mt-1 block w-full border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white dark:placeholder-slate-400 focus:border-indigo-500 dark:focus:border-indigo-500 focus:ring-indigo-500 dark:focus:ring-indigo-500 rounded-md shadow-sm text-sm" placeholder="Catatan tambahan..."></textarea>
                            </div>
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-slate-700 bg-white dark:bg-slate-800">
                                        @foreach ($transactions as $transaction)
                                            <tr class="hover:bg-gray-50/60 dark:hover:bg-slate-700/50 transition">
                                                <td class="px-6 py-4 text-gray-500 dark:text-slate-400 whitespace-nowrap">{{ $transaction->transaction_date->format('d M Y') }}</td>
                                                ...
                                                <td class="px-6 py-4 text-right font-bold {{ $transaction->type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }} whitespace-nowrap">
                                                    {{ $transaction->type === 'income' ? '+' : '-' }} Rp {{ number_format($transaction->amount, 0, ',', '.') }}
                                                </td>
                                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                                    <a href="{{ route('transactions.edit', $transaction) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 text-sm font-medium">Edit</a>
                                                    <button type="button" @click="showDeleteModal = true; deleteForm = '{{ route('transactions.destroy', $transaction) }}'" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 text-sm font-medium ml-3">Hapus</button>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                ...
                <div x-show="showDeleteModal"
                    ...
                    <div class="bg-gray-50 dark:bg-slate-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <form :action="deleteForm" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 dark:focus:ring-offset-slate-800 sm:ml-3 sm:w-auto sm:text-sm">
                                Hapus
                            </button>
                        </form>
                        <button type="button" @click="showDeleteModal = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-700 text-base font-medium text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-slate-800 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                            Batal
                        </button>
                    </div>
                </div>
