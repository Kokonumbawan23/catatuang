    ...
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gray-100 dark:bg-slate-900 px-4">
        ...
        <div class="w-full sm:max-w-md mt-6 px-6 py-6 bg-white dark:bg-slate-800 shadow-md overflow-hidden sm:rounded-lg">
            <form @submit.prevent="handleRegister" class="space-y-4">
                <div>
                    <label for="name" class="block font-medium text-sm text-gray-700 dark:text-slate-200">Nama</label>
                    <input
                        id="name"
                        v-model="name"
                        type="text"
                ...
                <div class="mt-4">
                    <label for="password_confirmation" class="block font-medium text-sm text-gray-700 dark:text-slate-200">Konfirmasi Password</label>
                    <input
                ...
                <div class="flex items-center justify-end mt-4">
                    <router-link to="/login" class="text-sm text-gray-600 hover:text-gray-900 dark:text-slate-400 dark:hover:text-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 me-4">
                        Sudah punya akun?
                    </router-link>
...
const handleRegister = async () => {
    ...
    try {
        ...
        if (messages) {
            ...
            error.value = e.response?.data?.message || 'Pendaftaran gagal.';
        }
    } finally {
        loading.value = false;
    }
